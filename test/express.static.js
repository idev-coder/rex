...
describe('rex.static()', function () {
  describe('basic operations', function () {
    ...
    it('should require root path', function () {
      assert.throws(rex.static.bind(), /root path required/)
    })

    it('should require root path to be string', function () {
      assert.throws(rex.static.bind(null, 42), /root path.*string/)
    })
  ...
  describe('fallthrough', function () {
    ...
    describe('when true', function () {
      ...
      it('should fall-through when URL too long', function (done) {
        ...
        app.use(rex.static(root, { 'fallthrough': true }))
    ...
    describe('when false', function () {
      ...
      it('should 404 when URL too long', function (done) {
        ...
        app.use(rex.static(root, { 'fallthrough': false }))
        app.use(function (req, res, next) {
          res.sendStatus(404)
        })
  ...
  describe('redirect', function () {
    before(function () {
      ...
      this.app.use(rex.static(fixtures))
    })
  ...
  describe('setHeaders', function () {
    before(function () {
      this.app = rex()
      this.app.use(rex.static(fixtures, { 'setHeaders': function (res) {
        res.setHeader('x-custom', 'set')
      } }))
    })

    it('should reject non-functions', function () {
      assert.throws(rex.static.bind(null, fixtures, { 'setHeaders': 3 }), /setHeaders.*function/)
    })
  ...
  describe('when index at mount point', function () {
    before(function () {
      this.app = rex()
      this.app.use('/users', rex.static(fixtures + '/users'))
    })
  ...
  describe('when mounted', function () {
    before(function () {
      this.app = rex()
      this.app.use('/static', rex.static(fixtures))
    })
  ...
  describe('when mounted "root" as a file', function () {
    before(function () {
      this.app = rex()
      this.app.use('/todo.txt', rex.static(fixtures + '/todo.txt'))
    })
  ...
  describe('when index file serving disabled', function () {
    before(function () {
      this.app = rex()
      this.app.use('/static', rex.static(fixtures, { 'index': false }))
      this.app.use(function (req, res, next) {
        res.sendStatus(404)
      })
    })
...
function createApp (dir, options, fn) {
  ...
  app.use(rex.static(root, options))
